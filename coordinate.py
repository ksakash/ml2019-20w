...
def get_soft_gradient (rho, lamda):
    if (rho+lamda < 0):
        return (rho+lamda)
    elif (rho-lamda > 0):
        return (rho-lamda)
...
def gradient_descent():
    ...
    for i in range (10000):
        ...
        #rho = get_coord_gradient_L2(w, idx)
        rho = np.dot(X_train[:,idx],(Y_train-np.dot(X_train, w)+w[idx]*X_train[:,idx]))
        z = np.linalg.norm(X_train[:,idx], 2)**2
        #delta = C_1*delta1 + C*delta2

        w[idx] = get_soft_gradient(rho, lamda)/z

        #w[idx] = w[idx] - alpha*delta

        loss_curr = get_loss (w)

        print ("loss_curr: ", loss_curr, " loss_prev: ", loss_prev)
